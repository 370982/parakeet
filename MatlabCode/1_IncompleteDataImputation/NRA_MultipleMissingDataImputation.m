...
figure(1);
plot(x,y,'r.');   
xlabel('Population(10000 persons)');ylabel('Passenger Traffic of Highway(million person- km)');
%----------------------------------------------------------------------
X=xlsread('demo.xls','B2:B61');     %population
Y=xlsread('demo.xls','D2:D61');     %convert missing value data 
figure(2);
plot(X,Y,'r.');    
xlabel('Population(10000 persons)');ylabel('ln(100000*Passenger Traffic of Highway)');
%parameter(a，b)---------------------------------------------------------
lxx=0;
lxy=0;
for i=1:length(X)      
...
alpha=0.05;     %Normal distribution test-------------------------------------
[mu,sigma]=normfit(Y);
